...
fea_8 = fea8(pd.concat([userComment_train, userComment_test]))
train = pd.merge(train, fea_8, on='userid', how='left')
test = pd.merge(test, fea_8, on='userid', how='left')

fea_9 = fea9(pd.concat([userComment_train, userComment_test]))
train = pd.merge(train, fea_9, on='userid', how='left')
test = pd.merge(test, fea_9, on='userid', how='left')

# fea_10 = fea10(pd.concat([userProfile_train, userProfile_test]))
# train = pd.merge(train, fea_10, on='userid', how='left')
# test = pd.merge(test, fea_10, on='userid', how='left')

fea_11 = fea11(pd.concat([action_train, action_test]))
train = pd.merge(train, fea_11, on='userid', how='left')
test = pd.merge(test, fea_11, on='userid', how='left')

fea_12 = fea12(pd.concat([action_train, action_test]))
